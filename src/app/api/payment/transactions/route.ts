...
export async function POST(request: NextRequest) {
  try {
    ...
    // Check gateway is configured before creating payment
    if (!process.env.FYARGO_API_KEY || !process.env.FYARGO_API_URL) {
      return NextResponse.json(
        { error: 'Payment gateway not configured. Contact admin to set up FYARGO_API_KEY.' },
        { status: 503 }
      );
    }
